...
"""
NOTES:
-Make it "wear object = description" or similar.
-Add RP to commands
-Pokemon Holder wearable that returns the number of pokeballs you have
and on closer look, the exact types of pokeballs.
-No underwear. That's default.
-can use type_limit to prevent types of clothes

?Only allows being covered by one object
"""
...
# The rest of these options have to do with clothing types. Clothing types are
# optional, but can be used to give better control over how different items of
# clothing behave. You can freely add, remove, or change clothing types to suit
# the needs of your game and use the options below to affect their behavior.

# The order in which clothing types appear on the description. Untyped 
# clothing or clothing with a type not given in this list goes last.
CLOTHING_TYPE_ORDER = ['hat', 'jewelry', 'top', 'undershirt', 'gloves', 
                       'fullbody', 'bottom', 'underpants', 'socks', 'shoes',
                       'accessory']

# The maximum number of each type of clothes that can be worn. Unlimited if 
# untyped or not specified.
CLOTHING_TYPE_LIMIT = {
    'hat': 1,
    'gloves': 1,
    'socks': 1,
...
# What types of clothes will automatically cover what other types of clothes
# when worn. Note that clothing only gets auto-covered if it's already worn
# when you put something on that auto-covers it - for example, it's perfectly
# possible to have your underpants showing if you put them on after your pants!
...
def order_clothes_list(clothes_list):
    ...
    Returns:
        ordered_clothes_list (list): The same list as passed, but re-ordered
                                     according to the hierarchy of clothing 
                                     types specified in CLOTHING_TYPE_ORDER.
    """
...
def single_type_count(clothes_list, type):
    """
    Returns an integer value of the number of a given type of clothing 
    in a list.
    ...
    Returns:
        type_count (int): Number of garments of the specified type in the 
                          given list of clothing objects
    """
    type_count = 0
    for garment in clothes_list:
        if garment.db.clothing_type:
...
class Clothing(DefaultObject):
    ...
    def wear(self, wearer, wearstyle, quiet=False):
        ...
        Args:
            ...
            wearstyle (True or str): string describing the style of wear 
                                     or True for none
        ...
        Notes:
            Optionally sets db.worn with a 'wearstyle' that appends a short 
            passage to the end of the name  of the clothing to describe how
            it's worn that shows up in the wearer's desc - I.E. 'around his
            neck' or 'tied loosely around her waist'. If db.worn is set to
            'True' then just the name will be shown.
        """
        ...
        to_cover = []
        if self.db.clothing_type and self.db.clothing_type \
                in CLOTHING_TYPE_AUTOCOVER:
