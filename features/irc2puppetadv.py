"""
IRC2Puppet Advanced - TO BE TESTED

Cloud_Keeper 2018

This is a more sophisticated version of the IRC2Puppet bot. This connects an 
Evennia location to an IRC channel, achieved by having a hidden 'listener'
...
This version has the additional feature of:
-In-game players may whisper IRC users with the 'whisper' command
-IRC users can whisper In-game players by PM'ing the IRC bot.
-IRC users can 'look' at the in-game room and room contents
...
_FACTORY_PATH = "typeclasses.irc2puppetadv.PortalBotFactory"
...
class DefaultPuppet(Character):
    ...
    **Puppet Object** -> AccountBot -> PortalBot -> IRC
    ...
    def at_object_creation(self):
        """
        Initialise attributes used by the Puppets.
        These values are overwritten on creation by the defaults set on the
        IRCBot, but are not automatically set after that. Use the default
        build commands in game to personalise the puppets.
        """
        self.db.desc = "This is overwritten by the AccountBot default values"
        self.db.enter = "This is overwritten by the AccountBot default values"
        self.db.exit = "This is overwritten by the AccountBot default values"
    ...
    def msg(self, text=None, **kwargs):
        ...
        Common message types that will be received by the listener are:
            ...
            Messages from the 'Look' Command and other in-game text:
                ...
                kwargs = {options:[]}
        """
        print("Account bot msg received " + str(text) + str(kwargs))
        ...
        if isinstance(text, tuple):
            ...
            if text[1]["type"] == "whisper":
                # Turn 'MUDUser whispers: "string"' to 'MUDUser: Hi There'
                msg = kwargs["from_obj"].key + ": " + msg.split('"', 1)[1][:-1]
                super(AccountBotOutputFunctions, self).msg(privmsg=((msg,), {"user": kwargs["user"]}))
                return

            if text[1]["type"] == "privmsg":
                # Messages by the bot itself. We do no formatting.
                super(AccountBotOutputFunctions, self).msg(privmsg=((msg,), {"user": kwargs["user"]}))
                return
...
##############################################################################
    ...
    def joined(self, channel):
        ...
        intact.
        """
        # Return user list to Account bot.
        self.get_nicklist()
        ...
        if kwargs["type"] == "nicklist":
            ...
            for nick in kwargs["nicklist"]:
                ...
                if nick in self.db.puppetdict:
                    ...
                    self.db.ev_location.msg_contents(self.db.puppetdict[nick].key + self.db.puppetdict[nick].db.exit)
...
#     the Irc2puppet Bot. Each user on the target IRC channel will be
#     represented by a Puppet object and all communication will be through
#     the puppet object using the execute_cmd method.
#     """
#     pass
...
class AccountBot(AccountBotInputFunctions, AccountBotOutputFunctions):
    """
    ...
    player). The AccountBot handles recieving in-game messages and formatting
    them to be sent to the player (PortalBot). The AccountBot also handles
    receiving external messages from the PortalBot and instigating the required
    reaction.
    """
    ...
    def start(self, ev_location=None, irc_botname=None, irc_channel=None,
        ...
        configdict = {"uid": self.dbid,
                      ...
                      "ssl": self.db.irc_ssl}
        _SESSIONS.start_bot_session(_FACTORY_PATH, configdict)
    ...
    def prep_listener(self):
        ...
        if listener:
            ...
            listener = create.create_object(DefaultListener, key=self.key + "-listener",
    ...
    def prep_puppet(self, nick):
        ...
        # Create a new puppet.
            ...
            puppetdict[nick] = create.create_object(DefaultPuppet, key=self.db.puppetprefix + nick + self.db.puppetsuffix,
                                                    location=self.db.ev_location)
